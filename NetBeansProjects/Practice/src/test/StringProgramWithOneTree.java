 ...
 * @author jayp
 */
public class StringProgramWithOneTree
{
    private OrderedTree _wordList;
    private Word[] _words;
    private int _noOfWords;   

    public StringProgramWithOneTree() throws Exception
    {     
    ...
    private void intiallizeWords(String given)
    {
        ...
        _wordList =new OrderedTree();
        for (int i = 0; i < _noOfWords; i++)
        {
            _wordList.putByName( str.nextToken());
        }
        _words= _wordList.inOrderTraversal();
        bubbleSort();
    } 

    private void bubbleSort()
    {
        for (int i = _wordList.size()-1; i >0; i--)
        {
            for (int j = 0; j < i; j++)
            {
    ...
    public static long main(String[] args) throws Exception
    {
        ...
        StringProgramWithOneTree str=new StringProgramWithOneTree();
        str.getInputFromFile();
//        System.out.println("Total no. of Words:"+str._noOfWords);
        long timeTaken=System.currentTimeMillis()-beginTime;        
//        str.printList();
        System.out.println("This Program uses trees");
        return timeTaken;
    }
